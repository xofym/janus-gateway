 ...
 * 		video = mcu-video.mjr
 *
 * \section recplayapi HLS API
 *
 * The HLS API supports several requests, some of which are
...
janus_hls_frame_packet *janus_hls_get_frames(const char *dir, const char *filename);
...
typedef struct janus_hls_session {
	...
	janus_hls_frame_packet *aframes;	/* Audio frames (for playout) */
	janus_hls_frame_packet *vframes;	/* Video frames (for playout) */
...
void janus_hls_update_recordings_list(void);
static void *janus_hls_playout_thread(void *data);
...
static int janus_hls_generate_offer(janus_hls_recording *rec) {
	if (rec == NULL) {
		return -1;
	}
...
int janus_hls_init(janus_callbacks *callback, const char *config_path) {
	...
	janus_hls_update_recordings_list();
...
struct janus_plugin_result *janus_hls_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep) {
	...
	if (!strcasecmp(request_text, "update")) {
		...
		json_object_set_new(response, "hls", json_string("ok"));
		goto plugin_response;
		...
		/* Send info back */
		response = json_object();
		json_object_set_new(response, "hls", json_string("list"));
		...
		goto plugin_response;
	} else if (!strcasecmp(request_text, "record") || !strcasecmp(request_text, "play") || !strcasecmp(request_text, "start") || !strcasecmp(request_text, "stop")) {
...
void janus_hls_setup_media(janus_plugin_session *handle) {
	...
	if (!session->recorder) {
		GError *error = NULL;
		janus_refcount_increase(&session->ref);
		g_thread_try_new("hls playout thread", &janus_hls_playout_thread, session, &error);
		if (error != NULL) {
			janus_refcount_decrease(&session->ref);
			/* FIXME Should we notify this back to the user somehow? */
			JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the HLS playout thread...\n", error->code, error->message ? error->message : "??");
	...
	if (session->recorder) {
		if (session->recording) {
			...
			if (file == NULL) {
				...
				/* Generate the offer */
				if (janus_hls_generate_offer(session->recording) < 0) {
...
recdone:
			...
			if (msg_sdp) {
				...
				error_code = JANUS_HLS_ERROR_INVALID_ELEMENT;
			...
			if (rec == NULL || rec->offer == NULL || g_atomic_int_get(&rec->destroyed)) {
				...
				error_code = JANUS_HLS_ERROR_NOT_FOUND;
				g_snprintf(error_cause, 512, "No such recording");
				goto error;
			}
			/* Access the frames */
			if (rec->arc_file) {
				session->aframes = janus_hls_get_frames(recordings_path, rec->arc_file);
			...
			if (rec->vrc_file) {
				session->vframes = janus_hls_get_frames(recordings_path, rec->vrc_file);
			...
			if (session->aframes == NULL && session->vframes == NULL) {
				error_code = JANUS_HLS_ERROR_INVALID_RECORDING;
...
error:
	...
	JANUS_LOG(LOG_VERB, "LeavingHLS handler thread\n");
...
void janus_hls_update_recordings_list(void) {
	...
	while ((recent = readdir(dir))) {
		...
		janus_hls_recording *rec = g_hash_table_lookup(recordings, &id);
		if (rec != NULL) {
		...
		if (audio && audio->value) {
			...
			rec->acodec = janus_audiocodec_from_name(janus_hls_parse_codec(recordings_path, rec->arc_file));
		}
		if (video && video->value) {
			...
			/* Check which codec is in this recording */
			rec->vcodec = janus_videocodec_from_name(janus_hls_parse_codec(recordings_path, rec->vrc_file));
		}
		rec->audio_pt = AUDIO_PT;
		if (rec->acodec != JANUS_AUDIOCODEC_NONE) {
			/* Some audio codecs have a fixed payload type that we can't mess with */
		...
		rec->video_pt = VIDEO_PT;
		rec->viewers = NULL;
		if (janus_hls_generate_offer(rec) < 0) {
			JANUS_LOG(LOG_WARN, "Could not generate offer for recording %"SCNu64"...\n", rec->id);
		}
		g_atomic_int_set(&rec->destroyed, 0);
	...
	if (old_recordings != NULL) {
		while (old_recordings != NULL) {
			...
			janus_hls_recording *old_rec = g_hash_table_lookup(recordings, &id);
			if (old_rec != NULL) {
				/* Remove it */
...
janus_hls_frame_packet *janus_hls_get_frames(const char *dir, const char *filename) {
	...
	offset = 0;
	janus_hls_frame_packet *list = NULL, *last = NULL;
	while (offset < fsize) {
		/* Read frame header */
		fseek(file, offset, SEEK_SET);
		bytes = fread(prebuffer, sizeof(char), 8, file);
		...
		janus_hls_frame_packet *p = g_malloc(sizeof(janus_hls_frame_packet));
		...
		if (list == NULL) {
			...
			int added = 0;
			janus_hls_frame_packet *tmp = last;
			while (tmp) {
				if (tmp->ts < p->ts) {
	...
	janus_hls_frame_packet *tmp = list;
	count = 0;
	while (tmp) {
		count++;
...
static void *janus_hls_playout_thread(void *data) {
	janus_hls_session *session = (janus_hls_session *)data;
	if (!session) {
	...
	janus_refcount_increase(&session->recording->ref);
	janus_hls_recording *rec = session->recording;
	if (session->recorder) {
		janus_refcount_decrease(&rec->ref);
		janus_refcount_decrease(&session->ref);
	...
	janus_hls_frame_packet *audio = session->aframes, *video = session->vframes;
	...
	while (!g_atomic_int_get(&session->destroyed) && session->active && !g_atomic_int_get(&rec->destroyed) && (audio || video)) {
		if (!asent && !vsent) {
	...
	janus_hls_frame_packet *tmp = NULL;
